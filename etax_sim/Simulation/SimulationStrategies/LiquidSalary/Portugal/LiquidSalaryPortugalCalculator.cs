...
namespace eTaxSim.Simulation.SimulationStrategies.LiquidSalary.Portugal
{
    internal class LiquidSalaryPortugalCalculator
    {
        ...
        {
            Model.BaseTwelfthsChristmasSubsidy =
                CalculateSubsidy(Model.ChristmasSubsidy, Model.Twelfths, Model.BaseSalary);
        ...
        {
            var result = aValue;
            var irsDeduct = ApplyTax(aValue, Model.IRS);
            var ssDeduct = ApplyTax(aValue, Model.SS);
        ...
        {
            var result = aSalary * aTaxPercent;
            return result;
        }
    }
}